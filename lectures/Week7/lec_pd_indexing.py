...
x  = x = ser.loc['2020-01-03':'2020-01-10']
print(x)
...
#select the values for the row indexes ['b','c','d','e'] for col1
...
#select the values for the row indexes ['b','d','f','j'] for col1 from the following dataframe

x = df.sort_index(inplace=True)
answer = df.loc[['b','d','f','j'], ['col1']]
print(answer)
...
#select the value for the even-indexed row col2 from the following dataframe

rows=list(range(0,len(df),2))  #get the even row indexes of the dataframe
result= df.iloc[rows, [1]]
print(result)

#x=df.iloc['2','4','6','8','10','12','14','16','18','col2'] #error: Too many indexers
...
#inplace=True (replace original copy)