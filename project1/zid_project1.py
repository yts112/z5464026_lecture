...
# ----------------------------------------------------------------------------
ROOTDIR = os.path.join(os.getcwd(), 'project1')
...
def read_dat(tic):
    ...
    step4_file = os.path.join(DATDIR, f"{tic}_prc.dat")
    with open(step4_file, mode='r') as b:
        lines = []
        for line in b:
            line = line.rstrip('\n')
            lines.append(line)
...
def line_to_dict(line):
    ...
    for col, width in COLWIDTHS.items():
        ...
        sstr1 = line[start:end]
        value = sstr1.strip()
        step5_dict[col] = value
        start = end
    return step5_dict
        ...
        if not tickers_lst:
            ...
            for ticker in tickers_lst:
                if ticker not in tic_exchange_dic:
                    raise Exception(f"{ticker} does not correspond to a key in tic_exchange_dic")
            tickers_lst = [ticker for ticker in tickers_lst]
    else:
...
def create_data_dict(tic_exchange_dic, tickers_lst=None, col_lst=None):
    ...
    for ticker in tickers_lst:
        ...
        for line in read_dat(ticker):
            dict82 = line_to_dict(line)

            newdict = {}
            for key in dict82:
                if key in col_lst:
                    newdict[key] = dict82[key]
            dict82 = newdict
            
            data.append(dict82)

        dict8[ticker] = {'exchange': exchange, 'data': data}
...
def create_json(data_dict, pth):
    ...
    # <COMPLETE THIS PART>
    step9_dict = {}
    with open(pth, mode="w") as step9_file:
        json.dump(step9_dict, step9_file)
